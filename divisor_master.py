    ...
    while n!=1:
        for i in range(2, n+1):
            if n % i == 0:
                ...
                break
        n=int(n/i)
# Все, что ниже, это просто для красивого вывода результата в виде "степени" простых чисел, а не просто произведения
# то есть чтобы вместо 2*3*3*3*3*5*5 выводилось 2*3^4*5^2
# Наверно, азмудрено, но проще не смог
    deliteli_1 = {}
    for w in deliteli:
        deliteli_1[w] = 0
    for w in deliteli:
